...
AJnRCharacter::AJnRCharacter(const class FPostConstructInitializeProperties& PCIP)
	: Super(PCIP)
{
...
void AJnRCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	// set up gameplay key bindings
	InputComponent->BindAction("Jump", IE_Pressed, this, &ACharacter::Jump);
	InputComponent->BindAxis("MoveRight", this, &AJnRCharacter::MoveRight);

	InputComponent->BindTouch(IE_Pressed, this, &AJnRCharacter::TouchStarted);
}

void AJnRCharacter::MoveRight(float Value)
{
	// add movement in that direction
	AddMovementInput(FVector(0.f,-1.f,0.f), Value);
}

void AJnRCharacter::TouchStarted(const ETouchIndex::Type FingerIndex, const FVector Location)
{
	// jump on any touch
	Jump();
}
